...
from .utils import DynamicsConstraints, EntityRef, TimeReference, TrafficSignalController, convert_bool, TransitionDynamics, CatalogReference, TrafficDefinition, Environment, AbsoluteSpeed, RelativeSpeedToMaster, convert_float, convert_int
...
class SpeedProfileAction(_PrivateActionType):
    ...
    def __init__(self,speeds, following_mode, times = None, dynamics_constraint=None, entity=None):
        ...
        if dynamics_constraint and not isinstance(dynamics_constraint,DynamicsConstraints):
    ...
    def __eq__(self,other):
        if isinstance(other,SpeedProfileAction):
            if self.get_attributes() == other.get_attributes() and \
                self.dynamics_constraint == other.dynamics_constraint and \
    ...
    def parse(element):
        ...
        if speed_profile_element.find('DynamicConstraints') is not None:
            dynamics_constraint = DynamicsConstraints.parse(speed_profile_element.find('DynamicConstraints'))
        ...
        return SpeedProfileAction(speeds,followmode,times,dynamics_constraint,entity)
    ...
    def get_element(self):
        ...
        if not self.isVersion(minor=2):
            raise OpenSCENARIOVersionError('SpeedProfileAction was introduced in OpenSCENARIO V1.2')
        element = ET.Element('PrivateAction')
        longaction = ET.SubElement(element,'LongitudinalAction')
        speedaction = ET.SubElement(longaction,'SpeedProfileAction',attrib=self.get_attributes())
